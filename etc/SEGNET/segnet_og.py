...
def segnet(input_shape=(256,256,1), n_labels=1, kernel=3, pool_size=(2, 2), output_mode="sigmoid"):
   # Level-1
    ...
    # conv_10 = tf.where(tf.greater(conv_10, tf.ones(shape=conv_10.shape)*0.5), 1.0, 0.0)
    c10 = conv_10.eval(session=tf.Session())
    np.where(c10>0.5, 1.0, 0)
    tf.convert_to_tensor(c10)
    unpool_2 = concatenate([unpool_2, c10],axis=-1)
    ...
    # conv_10 = tf.where(tf.greater(conv_10, tf.ones(shape=conv_10.shape)*0.5), 1.0, 0.0)
    c7 = conv_7.eval(session=tf.Session())
    np.where(c7>0.5, 1.0, 0)
    tf.convert_to_tensor(c7)
    
    unpool_3 = concatenate([unpool_3, c7],axis=-1)
    ...
    # conv_4.where(conv_4>.5,1.0,0.0)
    c4 = conv_4.eval(session=tf.Session())
    np.where(c4>0.5, 1.0, 0)
    tf.convert_to_tensor(c4)
    
    unpool_4 = concatenate([unpool_4, c4],axis=-1)
    ...
    # conv_5.where(conv_5>.5,1.0,0.0)
    c5 = conv_5.eval(session=tf.Session())
    np.where(c5>0.5, 1.0, 0)
    tf.convert_to_tensor(c5)

    unpool_5 = concatenate([unpool_5, c5],axis=-1)
